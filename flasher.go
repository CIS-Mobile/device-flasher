...
func main() {
	...
	if device == "" {
		...
		if device == "" {
			_, _ = fmt.Fprintln(os.Stderr, "Cannot determine device model. Exiting...")
			os.Exit(1)
		}
	}
...
func checkPrerequisiteFiles() {
	...
	if altosImage == "" {
		_, _ = fmt.Fprintln(os.Stderr, "Cannot continue without altOS device image. Exiting...")
		os.Exit(1)
	}
}
...
func flashDevices() {
	for _, device := range devices {
		...
		err := platformToolCommand.Run()
		if err != nil {
			_, _ = fmt.Fprintln(os.Stderr, "Failed to unlock device bootloader. Exiting...")
			return
		}
		time.Sleep(5 * time.Second)
	...
	if altosKey != "" {
		for _, device := range devices {
			...
			err := platformToolCommand.Run()
			if err != nil {
				_, _ = fmt.Fprintln(os.Stderr, "Failed to erase avb_custom_key. Exiting...")
				return
			}
			platformToolCommand = *fastboot
			...
			err = platformToolCommand.Run()
			if err != nil {
				_, _ = fmt.Fprintln(os.Stderr, "Failed to flash avb_custom_key. Exiting...")
				return
			}
			fmt.Println("Please use the volume and power keys on the device to confirm.")
			platformToolCommand = *fastboot
